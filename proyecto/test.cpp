...
void dibujaVehiculo() {
	...
	//Si recoge teera tamano incrementa... hasta un max
	float tamano = 0.3*log(monedas + 1)*0.5;
	if (tamano > 0.4) {
		tamano = 0.4;
	...
	if (MODO_ALAMBRICO) glutWireSphere(tamano, 30, 30); else glutSolidSphere(tamano, 30, 30);
	glPopMatrix();
